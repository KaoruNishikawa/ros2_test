...
node_name = "communication_test_sub"
...
class communication_test_sub(object):
    def __init__(self):
        ...
        subscriber = self.node.create_subscription(Float64, "/test/time", self.sub_callback, 1)

    def sub_callback(self, timer):
        current_time = float(time.time())
        send_time = float(timer.data)
        delta = current_time - send_time
        with open("/home/amigos/Documents/test_timer.txt", "a") as f:
            f.write(str(delta)+'\n')
