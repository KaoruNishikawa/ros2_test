...
launch_file=launch_topic_num_test.py
...
sleep 1s
# 
for topic_num in 1 20 40 60 80 100 120 140 160 180 200 220 240 260 280 300 320 340 360 380 400
do
    sed -i "s/{'topic_num'.*/{'topic_num'\: ${topic_num}}/g" $launch_file
    sleep 1s
    timeout -s SIGINT 100s ros2 launch $launch_file
    sleep 30s
    cd ~/Documents
    topic_num=`printf "%03g" $topic_num`
    mv -i cpu_used.txt cpu_used_$topic_num.txt && :
    mv -i mem_used.txt mem_used_$topic_num.txt && :
    mv -i net_count.txt net_count_$topic_num.txt && :
    mv -i test_node_num_0.txt delay_${topic_num}_node.txt && :
